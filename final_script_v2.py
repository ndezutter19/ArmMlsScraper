...
def delete_old_items():
    ...
    scan_kwargs = {
        'FilterExpression': Attr('listing_date').lt(one_year_ago.strftime(date_format)),
    ...
    print("Starting to scan and delete items with old listing dates...")

    with tqdm.tqdm(desc="Deleting old items", unit="item") as pbar:
        while not done:
            if start_key:
    ...
    print(f"Total old items deleted: {total_deleted}")
